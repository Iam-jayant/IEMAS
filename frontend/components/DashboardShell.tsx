  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center font-sans">
        <div className="text-text-3 font-mono text-xs uppercase tracking-widest animate-pulse">
          Initializing System...
        </div>
      </div>
    );
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-border">
          {isSidebarOpen && (
            <div className="flex items-center gap-2 pl-2">
              ...
              <div>
                <h1 className="text-text-1 font-bold font-display text-base tracking-tight leading-none">
                  IEMAS
                </h1>
                <span className="text-text-3 font-mono text-[9px] font-bold uppercase tracking-wider">
                  v1.0.0 {DEV_MODE && '(DEV)'}
                </span>
              </div>
            </div>
        ...
        <div className="border-t border-border p-4 bg-surface-2/40">
          {isSidebarOpen ? (
            <div className="mb-3 pl-1">
              ...
              <p className="text-text-3 text-[10px] uppercase font-bold tracking-wider font-mono mt-0.5">
                Energy Engineer
              </p>
            </div>
          ) : null}
        ...
        <header className="h-16 bg-surface border-b border-border flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="relative p-2 hover:bg-surface-2 rounded-xl text-text-2 hover:text-text-1 transition-colors cursor-pointer border border-border">
              <Bell size={16} />
              <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-red-accent rounded-full" />
            </button>
          </div>
        </header>

        <div className="flex-1 overflow-y-auto p-6 bg-bg">{children}</div>
      </main>
    </div>
  );
}